import { ChangeEvent, useState } from "react";
import DarkModeSwitch from "./DarkModeSwitch";

import Countdown from "./CountDown";
import { WebsiteName } from "./WebsiteName";
import { useNavigate } from "react-router-dom";
import { useAppDispatch, useAppSelector } from "../app/hook";
import { Region, Crypto, SelectState } from "../types";


import {fetchByIdCurrecny} from '../features/currency/currencyAction'
const Header = () => {
  const { data } = useAppSelector((state) => state.currency);
  const navigate = useNavigate();
  const dispatch = useAppDispatch();

  const [selected, setSelection] = useState<SelectState>({
    quote_unit: "inr",
    base_unit: "btc",
  });

  const handleQuoteUnitChange = (event: ChangeEvent<HTMLSelectElement>) => {
    const selectedValue = event.target.value;
    setSelection((prevState) => ({ ...prevState, quote_unit: selectedValue }));
    dispatch(fetchByIdCurrecny(selected));
  };

  const handleBaseUnitChange = (event: ChangeEvent<HTMLSelectElement>) => {
    const selectedValue = event.target.value;
    setSelection((prevState) => ({ ...prevState, base_unit: selectedValue }));
    dispatch(fetchByIdCurrecny(selected));

  };

  return (
    <nav className="w-full md:flex md:px-8 lg: justify-between">
      <WebsiteName />
      <div
        id="currency-selection"
        className="w-full md:w-1/3 flex justify-center items-center flex-wrap "
      >
        {/* <label htmlFor='mySelect'>INR</label> */}
        <select
          id="countrycurrancy"
          className="btn-group custom-select"
          value={selected.quote_unit}
          onChange={handleQuoteUnitChange}
        >
          {data &&
            data?.region?.map((item: Region, index: number) => (
              <option className="uppercase" key={index} value={item.quote_unit}>
                {item.quote_unit}
              </option>
            ))}
        </select>
        <select
          id="curptyocurrancy"
          className="btn-group custom-select"
          onChange={handleBaseUnitChange}
          value={selected.base_unit}
        >
          <option value="BTC">BTC</option>
          {data &&
            data?.crypto?.map((item: Crypto, index: number) => (
              <option value={item.base_unit} key={index}>
                {item.base_unit}
              </option>
            ))}
        </select>
        <button className=" btn-group ">BUY {selected.base_unit}</button>
      </div>
      <div
        id="social-link"
        className="mx-4  my-0 flex justify-center items-center flex-wrap gap-2 md:gap-0 md:w-fit md:mx-0"
      >
        <Countdown />

        <div
          onClick={() => navigate("/contact")}
          className="bg-[#3dc6c1] cursor-pointer h-8 w-fit  p-2 rounded-lg flex  justify-center items-center font-oswald text-sm text-white"
        >
          <img
            className="h-[100%] mr-2 inline-block"
            src="data:image/png;base64,"
          />
          Connect Telegram
        </div>

        <DarkModeSwitch />
      </div>
    </nav>
  );
};

export default Header;
